...
import { rafThrottle, isFirefox } from 'element-plus/lib/utils/util'
import { on, off } from 'element-plus/lib/utils/dom'
import { EVENT_CODE } from 'element-plus/lib/utils/aria'
...
export type ImageViewerAction = 'zoomIn' | 'zoomOut' | 'clocelise' | 'anticlocelise' | 'delete'
    ...
    function handleActions (action: ImageViewerAction, options = {}) {
      ...
      switch (action) {
        ...
        case 'delete':
          ...
          const that = this
          that.$confirm('是否要删除该图片?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            const list = JSON.parse(JSON.stringify(that.urlList))
            const delIndex = that.index
            list.splice(that.index--, 1)
            if (list.length === 0) {
              that.hide()
            }
            if (that.index < 0) {
            ...
            // @ts-ignore
            emit('update:urlList', list)
            emit('change', { list, index: delIndex })
          }).catch(() => {
            that.$message({
              type: 'info',
              message: '已取消删除'
